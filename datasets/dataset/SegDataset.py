...
class SegDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # 颜色迁移图像背景不为0
        # clipimg_dir = r'/data/data/update/256_128/train/image'
        # clipimg_data = cv2.imread(osp.join(clipimg_dir, nameid))
...
class SegEdgeDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        if self.masks_dir is not None:
            mask_data = self.convert_label(cv2.imread(osp.join(self.masks_dir, maskname), 0))
            edge_data = self.convert_label(cv2.imread(osp.join(self.edges_dir, maskname), 0))
            if self.mask_onehot:
                mask_data = np.identity(self.classes)[mask_data]
            # read data sample
            sample = dict(
                id=maskname,
                image=im_data,
                masks=[mask_data, edge_data])
            # apply augmentations   
            if self.transform is not None:
                sample = self.transform(**sample)
            sample['mask'] = pytorchtrans.ToTensor()(sample['masks'][0].astype('float32')).float()  # expand first dim for mask
            sample['edge'] = pytorchtrans.ToTensor()(sample['masks'][1].astype('float32')).float()
            sample.pop('masks')
            # sample['mask'] = sample['mask'][0]
