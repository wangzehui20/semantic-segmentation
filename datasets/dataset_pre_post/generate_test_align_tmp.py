...
def startest(imori_dir, imdst_dir, cfg, maskori_dir=None, maskdst_dir=None, maskrange=None):
    ...
    for name in tqdm(imlist, total=len(imlist)):
        ...
        end_idx, cliplist, statis = data_process(impath, imdst_dir, cfg, maskori_dir=maskori_dir,
                                                    maskdst_dir=maskdst_dir, start=start_idx, maskrange=maskrange, mode='test')
...
def folder():
    cfg = Config()
    imori_root_path = rf'{cfg.ORI_DIR}/val'
    names = os.listdir(imori_root_path)
    for name in names:
        imori_dir = rf"{cfg.ORI_DIR}/val/{name}/BDORTHO"
        maskori_dir = rf"{cfg.ORI_DIR}/val/{name}/UrbanAtlas"
        imclp_dir = rf"{cfg.CLIP_DIR}/val/{cfg.FILE_NAME}/{name}/image"
        maskclp_dir = rf"{cfg.CLIP_DIR}/val/{cfg.FILE_NAME}/{name}/mask"
        statis_path = rf"{cfg.CLIP_DIR}/val/{cfg.FILE_NAME}/{name}/test_statis.json"
        shiftul_path = rf"{cfg.CLIP_DIR}/val/{cfg.FILE_NAME}/{name}/test_shiftul.json"
        is_dir(imclp_dir)
        is_dir(maskclp_dir)
        ...
        shiftul, statis_dict = startest(imori_dir, imclp_dir, cfg, 
                                            maskori_dir=None, maskdst_dir=None, maskrange=None)
