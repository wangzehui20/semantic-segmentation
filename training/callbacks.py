...
class ModelCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        # only master gpu save checkpoints prevented spoiled
        if self.runner.local_rank == 0:
...
class Scheduler(Callback):
    ...
    def __init__(self, scheduler, freq='epoch', verbose=True, sc_name=None, monitor=None):
    ...
    def on_epoch_begin(self, epoch, logs=None):
        if self.verbose and self.runner.local_rank == 0:
    ...
    def on_epoch_end(self, epoch, logs=None):
        if self.freq == 'epoch':
            if self.sc_name=='ReduceLROnPlateauPatch':
                score = logs[self.monitor]
